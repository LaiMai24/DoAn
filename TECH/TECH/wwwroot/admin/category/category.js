(function ($) {
    ...
    self.RenderTableHtml = function (data) {
        ...
        if (data != "" && data.length > 0) {
            ...
            for (var i = 0; i < data.length; i++) {
                //var parentName ="" ;
                //if (self.ParentCategory != null && self.ParentCategory.length > 0 && parentName != "") {
                //    var findIndex = self.ParentCategory.findIndex(p => p.id == parentId.trim());
                //    if (findIndex >= 0) {
                //        var findCategory = self.ParentCategory.find(p => p.id == parentId.trim());
                //        parentName = findCategory.name
                //    } 
                //} 
                var item = data[i];
                html += "<tr>";
                ...
                //}
                html += "<td>" + (item.parentId != null && item.parentId > 0 ? item.parentName : item.name) + "</td>";
                html += "<td>" + (item.parentId == null && item.parentId <= 0 ? "" : item.name) + "</td>";                
        ...
        $.ajax({
            ...
            success: function (response) {
                if (response.Data != null && response.Data.length > 0) {
                    self.ParentCategory = response.Data;
    ...
    self.GetParentIdOrAll = function (sizeId, isRenderAPI) {
        var html = "";
        if (isRenderAPI) {
            html = "<select class=\"form-select form-control\" required disabled><option value=\"\"> Danh mục cấp 1 </option>";
        }
        else {
            html = "<select class=\"form-select form-control\" required><option value=\"\"> Chọn danh mục </option>";
        }
        if (self.ParentCategory != null && self.ParentCategory.length > 0) {
            for (var i = 0; i < self.ParentCategory.length; i++) {
                var item = self.ParentCategory[i];
                if (item.id == sizeId) {
    ...
    self.GetValue = function () {
        ...
        self.Category.parentId = $("#CategoryLevel1").val();
        //var parentName = $("#CategoryLevel1").val();
        //if (self.ParentCategory != null && self.ParentCategory.length > 0 && parentName != "") {
        //    var findIndex = self.ParentCategory.findIndex(p => p.name.trim() == parentName.trim());
        //    if (findIndex >= 0) {
        //        var findCategory = self.ParentCategory.find(p => p.name.trim() == parentName.trim());
        //        self.Category.parentId = findCategory.id;
        //    } else {
        //        self.Category.parentName = parentName;
        //    }
        //} else {
        //    self.Category.parentName = parentName;
        //}
    ...
    self.RenderHtmlByObject = function (view) {
        ...
        if (view.parentId != null && view.parentId > 0) {
            $("#CategoryLevel1").val(view.parentId);
        }
    }
