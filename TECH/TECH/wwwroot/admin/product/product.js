(function ($) {
                ...
                if (response.Data != null && response.Data.length > 0) {
                    for (var i = 0; i < response.Data.length; i++) {
                        ...
                        if (item.parentId == null) {
                            var datatchild = response.Data.filter(p => p.parentId == item.id);
                            if (datatchild != null && datatchild.length > 0) {
                                html += "<optgroup label='" + item.name + "'>";
                                for (var j = 0; j < response.Data.length; j++) {
                                    if (response.Data[j].parentId == item.id) {
                                        html += "<option value =" + response.Data[j].id + ">" + response.Data[j].name + "</option>";
                                    }
                                }
                                html += "</optgroup>";
                            } else {
    ...
    self.GetValue = function () {
        ...
        self.Product.description_short = CKEDITOR.instances.description_short.getData();
    ...
    self.RenderHtmlByObject = function (view) {
        ...
        CKEDITOR.instances.description_short.setData(view.description_short);
