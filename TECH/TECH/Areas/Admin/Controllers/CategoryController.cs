...
namespace TECH.Areas.Admin.Controllers
{
    public class CategoryController : BaseController
    {
        ...
        {
            var data = _categoryService.GetAllLevel1();
            return Json(new
            {
                Data = data
            });
        ...
        public JsonResult Add(CategoryModelView CategoryModelView)
        {
            ...
            {
                if (CategoryModelView.parentId.HasValue && CategoryModelView.parentId.Value > 0)
                {
                    //CategoryModelView.parentId = null;
                    //int id = _categoryService.AddPrent(CategoryModelView);
                    //CategoryModelView.parentId = id;
                ...
                //if (!string.IsNullOrEmpty(CategoryModelView.name))
                //{
                //    _categoryService.Add(CategoryModelView);
                //    _categoryService.Save();
                //}
        ...
        {
            var result = false;
            if (CategoryModelView.parentId == null)
            {
                result = _categoryService.UpdateParent(CategoryModelView);
            }
            else
            {
                result = _categoryService.Update(CategoryModelView);
            }
            _categoryService.Save();
            return Json(new
            {
        ...
        public JsonResult GetAllPaging(CategoryViewModelSearch categoryViewModelSearch)
        {
            ...
            if (data !=  null && data.Results != null && data.Results.Count > 0)
            {
                foreach (var item in data.Results)
                {
                    ...
                    {
                        var category = _categoryService.GetByid(item.parentId.Value);
                        if (category != null)
                        {
                            item.parentName = !string.IsNullOrEmpty(category.name)? category.name:"";
                        }
                    }
                }
            }
